    ...
    mongoclient.connect(this.url,function(err,db) {
        ...
        async.forEachOf(filters,function(filter,type,callback) {
           return filterc.insert({
            ...
            console.log(ids);
            datec.update({'date':date},{'date':date,'filters':ids},{'upsert':true},
            function(err) {
                if (err) {
                    return output(db,err);
                }
                console.log('Here');
                //TODO need to handle current updates to dates
                output(db);
            });    
        });

        /*
        filterc.insert(filters_data,function(err,result) {
            if (err) {
                return output(db,err);
            }
            console.log('herhe');
            assert.equal(filters_data.length,result.result.n);
            assert.equal(filters_data.length,result.ops.length);
            
            var ids = {};
            result.ops.forEach(function(filter_data) {
                ids[filter_data.type] = filter_data._id; 
            });
            //TODO Will need to handle case to handle filters of all types
            //TODO Need to handle updates to current filters(can be done 
            //     in dates collection)
            console.log(ids);
            datec.update({'date':date},{'date':date,'filters':ids},{'upsert':true},
            function(err) {
...
StoreClient.prototype.getFilter = function(olddate,type,callback) {
    ...
    mongoclient.connect(this.url, function(err,db) {
        ...
        var datesc = db.collection('dates'),
            filterc = db.collection('filters');
        console.log(olddate);
        filterc.find({'type':type,'lastUpdated':olddate}).toArray(function(err,res) {
            output(db,err,(!err && res && res.length > 0) ? res[0].filter : res);
        });
        /*
        datesc.find({'date':olddate}).toArray(function(err,result) {
            if (err) {
                return output(db,err);
            }
            console.log(result);
            console.log(type);
            var filterid = new ObjectID(result[0].filters[type]);
            return filterc.find({'_id':filterid}).toArray(function(err,res) {
                output(db,err,(!err) ? res[0].filter : res);    
            });
        });
        */
    });
