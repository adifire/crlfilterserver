...
StoreClient.prototype.getFilter = function() {
...
StoreClient.prototype.getCurrentFilter = function(type,callback) {
    this.getFilter('current',type,callback);
};
...
StoreClient.prototype.cleanFilterDB = function(callback) {
    ...
    mongoclient.connect(this.url,function(err,db) {
        if (err) return callback(new Error('Cannot connect to db. Error:' + err));
        var collection = db.collection('fitlers');
...
StoreClient.prototype.findDiff = function(date,type,callback) {
    var output = function(err,result) {
        if (!callback) return;
        return callback(err,result);
    };
