...
RedisStore.prototype.storeFilter = function(date,type,filter,callback) {
    var client = this.client,
        output = outputgen(callback),
        key = 'date:' + date + 'type:' + type;
    ...
    client.exists(key,function(err,res) {
        ...
        if (res !== 1) {
            client.set(key,filter,function(err) {
                if (err) {
                    return output(err);
                }
                client.expire(key,day,output);
            });
        }
    });
};

RedisStore.prototype.getFilter = function(date,type,callback) {
    var output = outputgen(callback);

    this.client.get('date:'+date + 'type:' + type,output);
};
